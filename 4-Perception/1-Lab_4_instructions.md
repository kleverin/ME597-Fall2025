...
2. The `object_detector.py` node must perform the following:
    ...
    * Print the x and y position of the centroid of a detected object (the red triangle), relative to the top left corner of the frame (e.g.- top left is (0,0), bottom right is (w,h)), in pixels. Note, you should continue detecting the triangle even when it is partially outside the screen. Other objects should not be detected during this time. When the triangle is completely out of the frame, no object should be detected. So only the red triangular object must be detected throughout the task.
